...
let adminApiToken = localStorage.getItem('belleruAdminToken') || '';
const apiBaseUrl = (window.BELLERU_API_BASE_URL || 'http://localhost:5000/api').replace(/\/$/, '');
...
async function parseApiResponse(response) {
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      adminApiToken = '';
      localStorage.removeItem('belleruAdminToken');
    }
...
function isBackendUnavailable(error) {
  return error?.backendUnavailable || error instanceof TypeError;
}

function backendUnavailableError(error) {
  const message = `Backend API is not reachable at ${apiBaseUrl}. Start the backend server or set BELLERU_API_BASE_URL to your deployed API.`;
  const nextError = new Error(message);
  nextError.backendUnavailable = true;
...
async function loginBackend(email, password) {
  ...
  adminApiToken = result.data.token;
  localStorage.setItem('belleruAdminToken', adminApiToken);
  return result.data;
}
...
async function uploadViaFirebaseStorage(files, folder) {
  const fileList = Array.from(files || []);
  return Promise.all(fileList.map((file, index) => uploadSingleFile(file, folder, index, fileList.length)));
}

async function uploadFiles(files, folder) {
  if (folder === 'banners') return uploadViaFirebaseStorage(files, folder);

  if (!await getAdminAuthToken()) throw new Error('Admin session expired. Please log in again.');

  const fileList = await Promise.all(Array.from(files || []).map(async (file) => {
    const preparedFile = await optimizeImageFile(file);
    if (preparedFile.size > 10 * 1024 * 1024) {
      throw new Error(`${preparedFile.name} is larger than 10MB. Please upload a smaller image.`);
    }
    return preparedFile;
  }));
  ...
  fileList.forEach((file) => formData.append(isCategory ? 'image' : 'images', file));

  try {
    const endpoint = isCategory ? '/uploads/category-image' : '/uploads/product-images';
    const result = await apiRequest(endpoint, { method: 'POST', body: formData });
    return isCategory ? [result.data.url] : result.data.urls;
  } catch (error) {
    if (isBackendUnavailable(error)) {
      toast('Backend unavailable. Uploading with Firebase Storage instead.');
      return uploadViaFirebaseStorage(fileList, folder);
    }
...
async function saveProduct(productId, payload) {
  try {
    ...
    if (productId) {
      await withTimeout(collection.doc(productId).set(firestorePayload, { merge: true }), 30000, 'Product update timed out. Check Firestore rules and connection.');
    } else {
      await withTimeout(collection.add({ ...firestorePayload, createdAt: firebase.firestore.FieldValue.serverTimestamp() }), 30000, 'Product publish timed out. Check Firestore rules and connection.');
    }
    toast('Backend unavailable. Product saved with Firebase Firestore.');
  }
}

async function saveCategory(id, payload) {
  try {
    if (id) await apiRequest(`/categories/${id}`, { method: 'PUT', body: JSON.stringify(payload) });
    else await apiRequest('/categories', { method: 'POST', body: JSON.stringify(payload) });
  } catch (error) {
    if (!isBackendUnavailable(error)) throw error;
    ...
    if (id) await db.collection('categories').doc(id).set(firestorePayload, { merge: true });
    else await db.collection('categories').add({ ...firestorePayload, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    toast('Backend unavailable. Category saved with Firebase Firestore.');
  }
}
...
function bindForms() {
  ...
  $('#productForm').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      toast(productId ? 'Product updated successfully' : 'Product published successfully');
  ...
  $('#categoryForm').addEventListener('submit', async (event) => {
    ...
    const payload = { name: $('#categoryName').value.trim(), sortOrder: Number($('#categorySort').value || 0), image: uploads[0] || categoryImageUrl };
    await saveCategory(id, payload);
    $('#categoryForm').reset();
    $('#categoryId').value = '';
  ...
  $('#confirmYes').addEventListener('click', async () => {
    if (pendingDelete) {
      if (pendingDelete.collection === 'products') {
        await apiRequest(`/products/${pendingDelete.id}`, { method: 'DELETE' });
      } else if (pendingDelete.collection === 'categories') {
        await apiRequest(`/categories/${pendingDelete.id}`, { method: 'DELETE' });
      } else {
  ...
  $('#loginForm').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      try {
        ...
        if (!isBackendUnavailable(error)) throw error;
        toast('Backend unavailable. Firebase admin mode is active.');
      }
    } catch (error) {
      toast(error.message);
    }
